...
class BlinkGlance extends LitElement {
    ...
    render() {
        ...
        return html`
            ${BlinkGlance.outerStyleTemplate}
            <ha-card>
            ${BlinkGlance.innerStyleTemplate}
            <div id="blink-wrapper" class="base-${this._v.aspectRatio}">
    ...
    updateStatuses( oldValue ) {
        ...
        if( this._v.captured === '' ) {
            ...
            const last = this.getState(this._s.isArmed, 0).state;
    ...
    updateMedia() {
        ...
        if( this._stream ) {
            ...
            if( this._s.imageFullDate.startsWith('capture/') ) { 
                this._s.imageDate = this.getState(this._s.isArmed,0).state;
    ...
    setConfig(config) {
        ...
        // see if blink prefix, remove from custom names if not present
        let prefix = "";
        if ( camera.startsWith( 'blink_','' ) ) {
            camera = camera.replace( 'blink_','' )
            prefix = "blink_"
        }
        if( config.prefix ) {
    ...
    async wsStartStream() {
        try {
            return await this._hass.callWS({
                type: this._v.playDirect ? "blink_stream_url" : "camera/stream",
                entity_id: this._s.cameraId,
            })
        } catch (err) {
            return null
...
s.onload = function() {
    ...
    s2.onload = function() {
        customElements.define('blink-glance', BlinkGlance);
